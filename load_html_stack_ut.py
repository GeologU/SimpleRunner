...
''')
        ...
        p = with_html_stack.DEV_PARAMS.inner
        self.assertEqual(p.as_text(['one line']), '    one line\n')
...
class TestHTMLRaw(unittest.TestCase):
    ...
    def test_text(self):
        ...
        self.assertEqual(
            raw.as_text(with_html_stack.DEV_PARAMS.inner),
            '    -p-some raw text-s-\n'
        )
        self.assertEqual(
            raw.as_text(with_html_stack.PROD_PARAMS),
            '-p-some raw text-s-'
        )
...
class TestHTMLAttribute(unittest.TestCase):
    ...
    def test_test(self):
        ...
        for attr, value, escape, expected in [
            ...
            with self.subTest(attr=attr, value=value, escape=escape, expected=expected):
                self.assertEqual(
                    with_html_stack.HTMLAttribute(attribute=attr, value=value, escape=escape).as_text(),
                    expected
                )
    ...
    def test_code(self):
        ...
        for attr, value, escape, expected in [
            ...
            with self.subTest(attr=attr, value=value, escape=escape, expected=expected):
                self.assertEqual(
                    with_html_stack.HTMLAttribute(attribute=attr, value=value, escape=escape).as_code(),
                    expected
                )
...
class TestHTMLTag(unittest.TestCase):
    ...
    def test_text(self):
        self.assertEqual(
            with_html_stack.HTMLTag('!a').as_text(params=with_html_stack.PROD_PARAMS),
            '<!a>'
        )
        self.assertEqual(
            with_html_stack.HTMLTag('a').as_text(params=with_html_stack.PROD_PARAMS),
        ...
        self.assertEqual(
            with_html_stack.HTMLTag('a', color='red').as_text(params=with_html_stack.PROD_PARAMS),
            '<a color="red"/>'
        )
...
'''
    ...
    def test_code(self):
        self.assertEqual(
            with_html_stack.HTMLTag('!a').as_code(params=with_html_stack.PROD_PARAMS),
            "doc('!a')"
        )
        self.assertEqual(
            with_html_stack.HTMLTag('a').as_code(params=with_html_stack.PROD_PARAMS),
        ...
        self.assertEqual(
            with_html_stack.HTMLTag('a', color='red').as_code(params=with_html_stack.PROD_PARAMS, raw='I am\n<p>inner html</p>.'),
...
class TestHTMLNode(unittest.TestCase):
    ...
    def test_code(self):
        ...
        node.node_tag = with_html_stack.HTMLTag('a', color='red')
        node.children = [with_html_stack.HTMLNode(tag=with_html_stack.HTMLTag('br'))]
        self.assertEqual(node.as_code(params=with_html_stack.DEV_PARAMS), "with doc('a', color='red'):\n    doc('br')\n")
...
class TestHTMLDocument(unittest.TestCase):
    ...
    def test_text_dev(self):
        self.assertEqual(self.doc.as_text(with_html_stack.DEV_PARAMS), '''\
<!DOCTYPE html>
<html lang="en">
    <head>
        <title>
...
''')
    ...
    def test_text_prod(self):
        self.assertEqual(self.doc.as_text(with_html_stack.PROD_PARAMS), '''\
<!DOCTYPE html><html lang="en"><head><title>Example Domain</title><meta charset="utf-8"/></head><body><div><h1>Example Domain</h1><p>This domain is for use in illustrative examples in documents. You may use this domain in literature without prior coordination or asking for permission.</p><p><a href="https://www.iana.org/domains/example">More information</a></p></div></body></html>''')
    ...
    def test_code(self):
        self.maxDiff = None
        self.assertEqual(self.doc.as_code(), '''\
...
''')
    ...
    def test_append(self):
        ...
        self.assertEqual(doc.as_text(with_html_stack.DEV_PARAMS), '''\
<!DOCTYPE html>
<html lang="en">
    <head>
        <title>
