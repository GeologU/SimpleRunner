 ...
 - [skeleton.py](#skeletonpy)
    ...
    - [with_html.py и его блочные тесты with_html_ut.py](#with_htmlpy-with_html_utpy)
...
#### with_html.py и его блочные тесты with_html_ut.py

Использую для генерации HTML.
Подобного рода библиотек много, мне было интересно написать самому.
В качестве примера использования смотрите конец `with_html_ut.py` и немногочисленные комментарии в самом `with_html.py`.
HTML генерируется, т.к. мне так проще писать вложенные теги.
Но главное, так гораздо проще вносить изменения.
Например, таблицу со списками заменить списком таблиц можно подвинув несколько строк в коде вместо переписывания всего HTML.

#### with_html_stack.py и его блочные тесты with_html_stack_ut.py

Дальнейшее развитие `with_html.py` с упрощённым интерфейсом.
В качестве примера использования смотрите тесты в `with_html_stack_ut.py`.
